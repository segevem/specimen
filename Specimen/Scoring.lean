...
-- Like GradedUniformDensityScore but distinguishes direct recursion
-- from mutual recursion. Direct recursive calls (Source.Rec) are
-- preferred over mutually recursive calls (Source.MutRec) because
-- direct recursion is structurally simpler and terminates more
-- predictably.
----------------------------------------------
...
def recAwareStepScorer : StepScorer RecAwareGradedScore := fun key memo inputVars step => do
  ...
  | .Check src polarity =>
    ...
    let speed := classifyCheckSpeed memo key src varDeps
    let likelihood ← classifyPassLikelihood inputVars key src polarity varDeps
    let recKind := match src with
      | .Rec .. => RecursionKind.Direct
      | .MutRec .. => RecursionKind.Mutual
      | .NonRec .. => RecursionKind.None
