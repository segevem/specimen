...
partial def searchBestScheduleM (ctorName : Name) (vars : List TypedVar)
  ...
  -- Dominance: only applied at leaves (complete orderings) to avoid incorrectly
  -- shadowing leaf scores with intermediate lower bounds.
  ...
  let scoreComponentOrdering := fun (env : List Name) (envSet : Std.HashSet Name)
    ...
    let score := bundle.scheduleScorer stepScores
    -- Dominance check: only at leaves (complete orderings).
    -- At intermediates, the same envKey can be reached by different hyp subsets
    -- with different remaining work, so dominance would incorrectly prune valid paths.
