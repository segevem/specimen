...
`Chain [] []`.  Candidate statements are checked with `specimen_test` first. -/
...
def sample (n : Nat) (size : Nat) : IO Unit := do
  let mut empties := 0
  let mut firstSum := 0
  let mut hist : Array Nat := Array.replicate (size) 0
  for i in [:n] do
    let l ← Gen.run (arbitrarySizedST (fun l => Chain [] [] l) size) size
  ...
  IO.println s!"first-name histogram [name:count]: {(List.range (size )).map (fun k => (k, hist[k]!))}"
